"""
Implementación nativa en Python puro del algoritmo de hash Tiger/192
(Ross Anderson y Eli Biham, 1995), usado por DC++ para identificar
ficheros mediante TTH (Tiger Tree Hash, ver `core/tth.py`) — igual que
`core/md4.py` para eD2k, no hay ningún build de OpenSSL habitual que
traiga Tiger en `hashlib`, así que no podemos depender de que esté
disponible en el sistema.

Los autores declaran Tiger libre de restricciones de uso y patentes,
utilizable con la implementación de referencia o con cualquier otra
que implemente el algoritmo correctamente. Las cuatro tablas S-box de
abajo (`_T`, 4×256 constantes de 64 bits, 8 KiB en total) son datos
matemáticos fijos de la propia especificación del algoritmo — no hay
forma de "reescribirlas" de otra manera y siguen siendo válidas
tomadas de cualquier implementación correcta; se han extraído y
verificado bit a bit contra la implementación de referencia en C de
RHash (dominio: rhash.sourceforge.net) comparando la salida de esta
clase con la de un pequeño programa C compilado ex profeso para varios
mensajes de prueba (cadena vacía, "abc", mensajes de más de un bloque
de 64 bytes...), no transcritas a mano.
"""
...
def _round(u: int, v: int, w: int, x: int, mul: int) -> tuple[int, int, int]:
    """Una `round` del compendio Tiger: `u` es la variable que se resta,
    `v` la que se suma y multiplica, `w` la que se combina con `x` de
    entrada y de la que se sacan los 8 bytes que indexan las S-box (ver
    `_T`). Los nombres u/v/w son posicionales, no a/b/c fijos: `_pass`
    llama a esta función rotando qué variable ocupa cada posición en
    cada una de las 8 rondas, igual que hace la macro `round(a,b,c,x,mul)`
    del original en C."""
    w = (w ^ x) & _MASK64
    u = (u - (
        _T[0][w & 0xFF] ^
...
def _key_schedule(x: list[int]) -> list[int]:
    x = list(x)
    x[0] = (x[0] - (x[7] ^ 0xA5A5A5A5A5A5A5A5)) & _MASK64
    x[1] = (x[1] ^ x[0]) & _MASK64
    x[2] = (x[2] + x[1]) & _MASK64
    x[3] = (x[3] - (x[2] ^ (((~x[1]) & _MASK64) << 19 & _MASK64))) & _MASK64
    x[4] = (x[4] ^ x[3]) & _MASK64
    x[5] = (x[5] + x[4]) & _MASK64
    x[6] = (x[6] - (x[5] ^ (((~x[4]) & _MASK64) >> 23))) & _MASK64
    x[7] = (x[7] ^ x[6]) & _MASK64
    x[0] = (x[0] + x[7]) & _MASK64
    x[1] = (x[1] - (x[0] ^ (((~x[7]) & _MASK64) << 19 & _MASK64))) & _MASK64
    x[2] = (x[2] ^ x[1]) & _MASK64
    x[3] = (x[3] + x[2]) & _MASK64
    x[4] = (x[4] - (x[3] ^ (((~x[2]) & _MASK64) >> 23))) & _MASK64
    x[5] = (x[5] ^ x[4]) & _MASK64
    x[6] = (x[6] + x[5]) & _MASK64
    x[7] = (x[7] - (x[6] ^ 0x0123456789ABCDEF)) & _MASK64
    return x


def _process_block(state: list[int], block: bytes) -> list[int]:
    ...
    return [(a ^ state[0]) & _MASK64, (b - state[1]) & _MASK64, (c + state[2]) & _MASK64]


class Tiger:
    """Interfaz compatible con hashlib: .update(data), .digest(),
    .hexdigest() — variante clásica (Tiger, no Tiger2: byte terminador
    de padding 0x01, no 0x80)."""
    ...
    def __init__(self, data: bytes = b"") -> None:
        self._buffer = b""
        self._length = 0
        self._state = [0x0123456789ABCDEF, 0xFEDCBA9876543210, 0xF096A5B4C3B2E187]
        if data:
            self.update(data)
    ...
    def update(self, data: bytes) -> None:
        ...
        while len(self._buffer) >= 64:
            self._state = _process_block(self._state, self._buffer[:64])
            self._buffer = self._buffer[64:]

    def _finalize_state(self) -> list[int]:
        """Aplica el padding Tiger estándar sobre una copia del estado y
        devuelve el estado final, sin mutar self (para poder llamar a
        digest() más de una vez, como hace hashlib)."""
        msg_len_bits = (self._length * 8) & _MASK64
        pad_len = (56 - (self._length + 1) % 64) % 64
        padding = b"\x01" + b"\x00" * pad_len + struct.pack("<Q", msg_len_bits)
        tail = self._buffer + padding
        state = list(self._state)
        for offset in range(0, len(tail), 64):
            state = _process_block(state, tail[offset:offset + 64])
        return state

    def digest(self) -> bytes:
        return struct.pack("<3Q", *self._finalize_state())
